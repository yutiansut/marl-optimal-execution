...
if PRINT_HISTORICAL:
    ...
    hist_file = "/nethome/cb107/emh/data/trades/trades_{}/ct{}_{}.bgz".format(
        hist_year, "m" if int(hist_year) > 2014 else "", hist_date
    )
...
if PRINT_HISTORICAL:
    ...
    # df_sim = df_sim.reindex(rng,method='ffill')
    # df_sim = df_sim.reset_index(drop=True)
    (df_sim["PRICE"] / 1000).plot(color="darkblue", grid=True, linewidth=LW, alpha=0.9, ax=axes[0])
    ...
    for idx in df_agent.index:
        ...
        if symbol in event:
            ...
            if shares > 0:
                ...
                axes[0].axvline(x=idx, linewidth=LW + 3, color="green", linestyle="dashed")
                axes[0].legend(["Simulated"])
            elif shares < 0:
                print("SHORT at {}".format(idx))
                axes[0].axvline(x=idx, linewidth=LW + 3, color="orange", linestyle="dashdot")
            else:
                print("EXIT at {}".format(idx))
                axes[0].axvline(x=idx, linewidth=LW + 3, color="red", linestyle="dotted")
        else:
            print("EXIT at {}".format(idx))
            axes[0].axvline(x=idx, linewidth=LW + 3, color="red", linestyle="dotted")

plt.suptitle("Simulated price for {} stock".format(symbol), y=0.925)

axes[0].set_ylabel("Price")

colors = ["darkblue", "green", "orange", "red"]
styles = [None, "dashed", "dashdot", "dotted"]
lines = [Line2D([0], [0], color=c, linestyle=l, linewidth=3) for c, l in zip(colors, styles)]
labels = ["Stock price", "Long Order", "Short Order", "Exit Order"]
axes[0].legend(lines, labels, loc="upper center", prop={"size": PLT_SIZE - 4})

if PRINT_VOLUME:
    axes[1].set_xlabel("Execution Time (from 09:30 to 16:00)")
    axes[1].set_ylabel("Executed Volume (units)")
    axes[0].get_xaxis().set_visible(False)
else:
    axes[0].set_xlabel("Time (from 09:30 to 16:00)")

# plt.savefig('background_{}.png'.format(b))

axes[0].set_xticks([])
axes[0].set_yticks([])
axes[0].set_xlim((737238.3953566843, 737238.6663537572))
axes[0].set_ylim((df_sim["PRICE"].min() / 1000, df_sim["PRICE"].max() / 1000))

plt.savefig("graphs/{}_{}.png".format(sys.argv[0][4:-3], str(pd.Timestamp.now())), bbox_inches="tight")
# plt.show()
