...
if SHOW_BEST:
    ...
    for idx in df_book.index:
        ...
        if ba and bb:
            df_book.loc[idx, round((ba + bb) / 2)] = midpoint_value
...
if not TIME_STEPS:
    unstacked.index = unstacked.index.time
...
with sns.axes_style("white"):
    ax = sns.heatmap(
        unstacked,
        cmap="seismic",
        mask=unstacked < min_volume,
        vmin=min_volume,
        cbar_kws={"label": "Shares Available (units)"},
        center=0,
        antialiased=False,
    )

ax.set(xlabel="Quoted Price ($)", ylabel="Quote Time (from 09:30 to 16:00)")
...
if PLOT_HISTORICAL:
    ...
    ax = sns.heatmap(
        df, cmap=sns.color_palette("coolwarm", 7), cbar_kws={"label": "Shares Available (units)"}, center=0
    )
    ax.set_title("Limit Order Book")
    ax.set(xlabel="Quoted Price ($)", ylabel="Quote Time (from 09:30 to 16:00)")
...
# Show all the plots.
plt.savefig(
    "graphs/{}_{}.png".format(sys.argv[0][4:-3], str(pd.Timestamp.now())), bbox_inches="tight"
)  # do not save as svg!
# plt.show()
