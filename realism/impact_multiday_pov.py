...
def extract_seed_date_from_path(p, yes_base, no_base):
    ...
    if yes_no == "yes":
        ...
        pov = m.group(2)
        date = m.group(3)
        out_dict = {"type": yes_no, "seed": seed, "pov": pov, "date": date}
        return out_dict
    elif yes_no == "no":
        seed = m.group(1)
        date = m.group(2)
        out_dict = {"type": yes_no, "seed": seed, "date": date}
        return out_dict
    else:
...
def generate_plot_data_cache_dicts(log_dirs_no_glob, log_dirs_yes_glob, yes_base, no_base):
    ...
    for dn in log_dirs_no:
        ...
        no_yes_dict[dn] = {"seed": no_seed, "date": no_date, "execution_paths": []}
        ...
        for dy in log_dirs_yes:
            ...
            if yes_seed == no_seed and yes_date == no_date:
                no_yes_dict[dn]["execution_paths"].append({"path": dy, "pov": pov})
...
def get_differences(tup):
    ...
    out_dict = {"out_df": out_df, "impact_statistics": impact_statistics, "pov": pov}
...
def plot_aggregated(aggregated, plot_params_dict):
    ...
    axes.fill_between(
        x=aggregated.index,
        y1=aggregated["mid_price_median"],
        y2=aggregated["mid_price_95_q"],
        color="blue",
        alpha=plot_params_dict["alpha_90"],
        label="90%",
    )
    axes.fill_between(
        x=aggregated.index,
        y2=aggregated["mid_price_median"],
        y1=aggregated["mid_price_05_q"],
        color="blue",
        alpha=plot_params_dict["alpha_90"],
    )
    axes.fill_between(
        x=aggregated.index,
        y2=aggregated["mid_price_median"],
        y1=aggregated["mid_price_75_q"],
        color="blue",
        alpha=plot_params_dict["alpha_50"],
        label="50%",
    )
    axes.fill_between(
        x=aggregated.index,
        y2=aggregated["mid_price_median"],
        y1=aggregated["mid_price_25_q"],
        color="blue",
        alpha=plot_params_dict["alpha_50"],
    )
...
def aggregate_data(data_to_process, num_workers):
    ...
    for pov in POVs:
        ...
        aggregated_data.update({pov: aggregated})
...
def main(config_path, num_workers, recompute):
    ...
    if not params:
        print(
            "No files to process. Please check fields `yes_base`, `no_base`, `log_dirs_no_glob` and `log_dirs_yes_"
            "glob` in config file"
        )
        ...
        try:
            ...
            try:
                ...
                if not data_to_process:
                    raise FileNotFoundError("No individual cached files")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_workers", "-n", help="Number of cores to use in computation", required=False, default=8, type=int
    )
    parser.add_argument(
        "--recompute",
        "-r",
