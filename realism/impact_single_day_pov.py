...
def make_plots(plot_data, plot_params_dict):
    ...
    orderbook_df["MID_PRICE"].plot(
        ax=axes[0], label=plot_params_dict["baseline_label"], color=color, linestyle=linestyle
    )
    ...
    for plot_data_dict in plot_data:
        ...
        # mid_price
        orderbook_with_execution_df["MID_PRICE"].plot(
            ax=axes[0], label=f'{plot_params_dict["execution_label"]}{pov}', color=color, linestyle=linestyle
        )

        # normalised difference
        mid_price_yes_execution, mid_price_no_execution = forward_fill_series(
            orderbook_with_execution_df["MID_PRICE"], orderbook_df["MID_PRICE"]
        )
    ...
    axes[0].axvspan(
        pd.Timestamp(plot_params_dict["shade_start_datetime"]),
        pd.Timestamp(plot_params_dict["shade_end_datetime"]),
        alpha=0.2,
        color="grey",
    )
    axes[1].axvspan(
    ...
    fig.savefig(
        plot_params_dict["output_file_path"],
        format="png",
        dpi=300,
        transparent=False,
        bbox_inches="tight",
...
def prep_data(plot_data, cache_file, only_executed, clipped_start_time, clipped_end_time, plot_params_dict):
    ...
    for data_dict in plot_data:
        ...
        abides_orderbook_df = make_orderbook_for_analysis(
            data_dict["no_execution_exchange_path"], data_dict["no_execution_orderbook_path"], num_levels=1
        )
        abides_execution_orderbook_df = make_orderbook_for_analysis(
            data_dict["yes_execution_exchange_path"], data_dict["yes_execution_orderbook_path"], num_levels=1
        )
        ...
        stats_dict = compute_impact_statistics(
            abides_orderbook_df,
...
def main(config_path):
    ...
    global ONLY_EXECUTED, CLIPPED_START_TIME, CLIPPED_FINISH_TIME, PLOT_DATA, CACHE_FILE, USE_CACHE, PLOT_PARAMS_DICT, SPREAD_PLOT_LOG_SCALE, ORDERBOOK_IMBALANCE_PLOT_LOG_SCALE
    ...
    if USE_CACHE and os.path.exists(CACHE_FILE):
        ...
        out_data = prep_data(
            PLOT_DATA, CACHE_FILE, ONLY_EXECUTED, CLIPPED_START_TIME, CLIPPED_FINISH_TIME, PLOT_PARAMS_DICT
        )
