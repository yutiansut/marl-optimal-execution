...
all_metrics = [
    MinutelyReturns,
    AggregationNormality,
    Autocorrelation,
    VolatilityClustering,
    Kurtosis,
    VolumeVolatilityCorrelation,
    ReturnsVolatilityCorrelation,
]
...
def plot_metrics(samples_per_day, real_dir, sim_dirs, sim_colors, recompute):
    ...
    for my_metric in all_metrics:
        ...
        try:
            ...
            pass
        try:
            os.mkdir("visualizations/asset_returns")
        except:
            pass
        plt.savefig(
            "visualizations/asset_returns/{}_{}.png".format(my_metric.__class__.__name__, sim_name), bbox_inches="tight"
        )
        plt.clf()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Processes historical data and simulated stream files and produce plots"
    ...
    parser.add_argument(
        ...
        type=dir_path,
        required=False,
        help="Directory containing preprocessed" "historical data for asset return" "stylized facts",
    )
    parser.add_argument(
        "-s",
        "--simulated-data-dir",
