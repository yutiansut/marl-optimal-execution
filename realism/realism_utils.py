...
from util.formatting.convert_order_book import is_wide_book, process_orderbook
...
def get_trades(sim_file):
    ...
    df_vol = df["SIZE"].sum()
    ohlcv = pd.DataFrame({"open": df_open, "high": df_high, "low": df_low, "close": df_close, "volume": df_vol})
...
def make_orderbook_for_analysis(
    stream_path, orderbook_path, num_levels=5, ignore_cancellations=True, hide_liquidity_collapse=True
):
    ...
    columns = list(
        itertools.chain(
            *[
                [f"ask_price_{level}", f"ask_size_{level}", f"bid_price_{level}", f"bid_size_{level}"]
...
def find_nearest_ts_idx(df, np_dt64):
    ...
    df_index = df.index.get_loc(
        min(timestamps[upper_index], timestamps[upper_index - 1], key=lambda x: abs(x - np_dt64))
    )  # find the closest between upper and lower timestamp
    return df_index
...
def compute_impact_statistics(
    ...
    seed,
    experiment_name,
    spread_lookback="1min",
    execution_agent_name="POV_EXECUTION_AGENT",
    log_dir="../log",
):
    ...
    start_mid_price_orig, end_mid_price_orig, end_mid_price_execution = get_relevant_prices(
