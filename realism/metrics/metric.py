...
class Metric:
    ...
    def hist(
        ...
        simulated,
        real,
        title="Simulated vs. Historical Histogram",
        xlabel="Values",
        log=False,
        bins=75,
        clip=None,
        ...
        for k, v in simulated.items():
            if k[0] == "rmsc02":
                k = ("RMSC02", k[1])
            elif k[0] == "sparse_zi_100":
                k = ("Sparse ZI", k[1])
            plt.hist(v, bins=bins, color=k[1], log=log, alpha=1, label=k[0], histtype="step", linewidth=3)

        plt.title(
            title
        )  #  + (" (log scale)" if log else "")  + ("" if clip is None else " (clipped @ ±{})".format(clip)))
        plt.xlabel(xlabel)
    ...
    def line(
        self, simulated, real, title="Simulated vs. Historical", xlabel="X", ylabel="Y", logy=False, plot_real=True
    ):
        real = np.array(real)
        for k, v in simulated.items():
            simulated[k] = np.array(v)
