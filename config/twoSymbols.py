...
parser.add_argument("-g", "--greed", type=float, default=0.25, help="Impact agent greed")
parser.add_argument("-i", "--impact", action="store_false", help="Do not actually fire an impact trade.")
parser.add_argument(
...
parser.add_argument(
    "-r", "--shock_variance", type=float, default=500000, help="Shock variance for mean reversion process (sigma^2_s)"
)
...
symbols = {
    "SYM1": {"r_bar": 100000, "kappa": 0.05, "sigma_s": sigma_s},
    "SYM2": {"r_bar": 150000, "kappa": 0.05, "sigma_s": sigma_s},
...
for i, t1 in zip(range(latency.shape[0]), agent_types):
    for j, t2 in zip(range(latency.shape[1]), agent_types):
        ...
        if j > i:
            ...
            if ("ExchangeAgent" in t1 and "EtfArbAgent" in t2) or ("ExchangeAgent" in t2 and "EtfArbAgent" in t1):
                # latency[i,j] = 20000
                latency[i, j] = 5
            elif ("ExchangeAgent" in t1 and "EtfMarketMakerAgent" in t2) or (
                "ExchangeAgent" in t2 and "EtfMarketMakerAgent" in t1
            ):
                # latency[i,j] = 20000
                latency[i, j] = 1
            elif ("ExchangeAgent" in t1 and "ImpactAgent" in t2) or ("ExchangeAgent" in t2 and "ImpactAgent" in t1):
                # latency[i,j] = 20000
                latency[i, j] = 1
