...
def run_in_parallel(num_simulations, num_parallel, config, log_folder, verbose):
    ...
    processes = [
        f'python -u abides.py -c {config} -l {log_folder}_seed_{seed} {"-v" if verbose else ""} -s {seed}'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=None, help="Seed controlling the generated global seeds")
    parser.add_argument("--num_simulations", type=int, default=1, help="Total number of simulations to run")
    parser.add_argument("--num_parallel", type=int, default=None, help="Number of simulations to run in parallel")
    parser.add_argument("--config", required=True, help="Name of config file to execute")
    parser.add_argument("--log_folder", required=True, help="Log directory name")
