...
print("\n====== Experimental wallclock elapsed: {} ======\n".format(pd.Timestamp("now") - wallclock_start))
...
zi_strategy = [(0, 250, 1), (0, 500, 1), (0, 1000, 0.8), (0, 1000, 1), (0, 2000, 0.8), (250, 500, 0.8), (250, 500, 1)]
...
parser.add_argument(
    "-b", "--book_freq", default=None, help="Frequency at which to archive order book for visualization"
)
...
parser.add_argument("-o", "--log_orders", action="store_true", help="Log every order-related action by every agent.")
...
parser.add_argument("-f", "--obi_freq", type=int, default=1e9 * 60, help="OBI subscription frequency")
parser.add_argument("-r", "--flat_range", type=float, default=0.1, help="OBI dead zone for staying flat")
parser.add_argument("-y", "--entry_threshold", type=float, default=0.1, help="OBI imbalance to enter a position")
parser.add_argument("-t", "--trail_dist", type=float, default=0.05, help="OBI trailing stop distance to exit positions")
parser.add_argument("-e", "--levels", type=int, default=10, help="OBI order book levels to consider")
parser.add_argument(
    "-n", "--num_simulations", type=int, default=5, help="Number of consecutive simulations in one episode."
)
...
if log_dir is None:
    log_dir = str(int(pd.Timestamp("now").timestamp()))
...
print(
    "OBI Freq: {}, OB Levels: {}, Entry Thresh: {}, Trail Dist: {}".format(
...
symbol_seeds = {}
for sym in symbols:
    symbol_seeds[sym] = np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32))
        ...
        while n > 0:
            agents.append(
                ZeroIntelligenceAgent(
                    agent_id,
                    "ZI Agent {}".format(agent_id),
                    strat_name,
                    random_state=get_rand_obj(agent_seeds[agent_id]),
                    log_orders=log_orders,
                    symbol=symbol,
                    starting_cash=starting_cash,
                    sigma_n=zi_obs_noise,
                    r_bar=s["r_bar"],
                    kappa=s["agent_kappa"],
                    sigma_s=s["fund_vol"],
                    q_max=10,
    ...
    for i in range(num_mm):
        ...
        agents.extend(
            [
                MarketMakerAgent(
                    agent_id,
                    "Market Maker Agent {}".format(agent_id),
                    "MarketMakerAgent",
                    symbol=symbol,
                    starting_cash=starting_cash,
                    min_size=500,
    ...
    for i in range(num_mom):
        ...
        agents.extend(
            [
                MomentumAgent(
                    agent_id,
                    "Momentum Agent {}".format(agent_id),
                    "MomentumAgent",
                    symbol=symbol,
                    starting_cash=starting_cash,
                    min_size=1,
