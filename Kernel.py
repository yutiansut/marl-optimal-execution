...
class Kernel:
    ...
    def sendMessage(self, sender=None, recipient=None, msg=None, delay=0):
        ...
        if sender is None:
            raise ValueError(
                "sendMessage() called without valid sender ID", "sender:", sender, "recipient:", recipient, "msg:", msg
            )
        ...
        if msg is None:
            raise ValueError(
                "sendMessage() called with message == None", "sender:", sender, "recipient:", recipient, "msg:", msg
            )
    ...
    def setWakeup(self, sender=None, requestedTime=None):
        ...
        if sender is None:
            raise ValueError(
                "setWakeup() called without valid sender ID", "sender:", sender, "requestedTime:", requestedTime
            )
        ...
        log_print("Kernel adding wakeup for agent {} at time {}", sender, self.fmtTime(requestedTime))
    ...
    def setAgentComputeDelay(self, sender=None, requestedDelay=None):
        ...
        if not type(requestedDelay) is int:
            raise ValueError(
                "Requested computation delay must be whole nanoseconds.", "requestedDelay:", requestedDelay
            )

        # requestedDelay must be non-negative.
        if not requestedDelay >= 0:
            raise ValueError(
                "Requested computation delay must be non-negative nanoseconds.", "requestedDelay:", requestedDelay
            )
    ...
    def delayAgent(self, sender=None, additionalDelay=None):
        ...
        if not type(additionalDelay) is int:
            raise ValueError("Additional delay must be whole nanoseconds.", "additionalDelay:", additionalDelay)

        # additionalDelay must be non-negative.
        if not additionalDelay >= 0:
            raise ValueError("Additional delay must be non-negative nanoseconds.", "additionalDelay:", additionalDelay)
    ...
    def appendSummaryLog(self, sender, eventType, event):
        ...
        self.summaryLog.append(
            {"AgentID": sender, "AgentStrategy": self.agents[sender].type, "EventType": eventType, "Event": event}
        )
