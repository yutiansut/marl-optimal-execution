...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Processes DOW30 data into pickled pd.DataFrame files required as " "input for plotting."
    )
    parser.add_argument(
        "dow_data_dir", type=dir_path, help="Path to directory containing all of the DOW30 data in EFS."
    )
    ...
    parser.add_argument(
        "start_date", type=parse, help="First date of DOW30 data to use for a given symbol in format YYYYMMDD."
    )
    parser.add_argument(
        "end_date", type=parse, help="Final date of DOW30 data to use for a given symbol in format YYYYMMDD."
    )
