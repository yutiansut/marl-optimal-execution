...
from util.formatting.convert_order_stream import check_positive, dir_path, get_start_end_time, get_year_month_day
...
def process_orderbook(df, level):
    ...
    # nearly_lobster = pd.DataFrame(rows_list, index=unstacked.index)
...
def save_formatted_order_book(orderbook_bz2, ticker, level, out_dir="."):
    ...
    # orderbook_df.replace({9999999999: np.nan, -9999999999: np.nan}).to_csv(
    #     filename,
    #     header=sum(
    #         (
    #             ["ask_price_%s" % lvl, "ask_size_%s" % lvl, "bid_price_%s" % lvl, "bid_size_%s" % lvl]
