...
from util.util import be_silent, log_print
...
class OrderBook:
        ...
        while matching:
            ...
            if matched_order:
                ...
                self.owner.sendMessage(
                    matched_order.agent_id, Message({"msg": "ORDER_EXECUTED", "order": matched_order})
                )
                ...
                log_print(
                    "SENT: notifications of order acceptance to agent {} for order {}", order.agent_id, order.order_id
                )
        ...
        if not matching:
            ...
            if self.bids:
                self.owner.logEvent(
                    "BEST_BID",
                    "{},{},{}".format(
                        self.symbol, self.bids[0][0].limit_price, sum([o.quantity for o in self.bids[0]])
                    ),
                )
            ...
            if self.asks:
                self.owner.logEvent(
                    "BEST_ASK",
                    "{},{},{}".format(
                        self.symbol, self.asks[0][0].limit_price, sum([o.quantity for o in self.asks[0]])
                    ),
                )
    ...
    def executeOrder(self, order):
        ...
        if not book:
            ...
            for idx, orders in enumerate(self.history):
                if matched_order.order_id not in orders:
                    continue
            ...
            if self.isEqualPrice(order, o[0]):
                ...
                for ci, co in enumerate(book[i]):
                    if order.order_id == co.order_id:
                        ...
                        for idx, orders in enumerate(self.history):
                            if cancelled_order.order_id not in orders:
                                continue
                        ...
                        self.owner.sendMessage(
                            order.agent_id, Message({"msg": "ORDER_CANCELLED", "order": cancelled_order})
                        )
    ...
    def modifyOrder(self, order, new_order):
        ...
        if not self.isSameOrder(order, new_order):
            return
            ...
            if self.isEqualPrice(order, o[0]):
                for mi, mo in enumerate(book[i]):
                    if order.order_id == mo.order_id:
                        book[i][0] = new_order
                        for idx, orders in enumerate(self.history):
                            if new_order.order_id not in orders:
                                continue
                            ...
                            self.owner.sendMessage(
                                order.agent_id, Message({"msg": "ORDER_MODIFIED", "new_order": new_order})
                            )
        if order.is_buy_order:
            self.bids = book
        else:
    ...
    def get_transacted_volume(self, lookback_period="10min"):
        ...
        executed_transactions = unrolled_history_df[
            unrolled_history_df["transactions"].map(lambda d: len(d)) > 0
        ...
        unrolled_transactions = reduce(
            lambda col1, col2: pd.concat([col1, col2], axis=0),
        ...
        unrolled_transactions = unrolled_transactions.rename(columns={0: "execution_time", 1: "quantity"})
    ...
    def prettyPrint(self, silent=False):
        ...
        book += "Last trades: simulated {:d}, historical {:d}\n".format(
            ...
            self.owner.oracle.observePrice(
                self.symbol, self.owner.currentTime, sigma_n=0, random_state=self.owner.random_state
            ),
        )
