...
from realism.realism_utils import MID_PRICE_CUTOFF, make_orderbook_for_analysis
...
def create_orderbooks(exchange_path, ob_path):
    ...
    processed_orderbook = make_orderbook_for_analysis(
        exchange_path, ob_path, num_levels=1, hide_liquidity_collapse=False
    )
    cleaned_orderbook = processed_orderbook[
        (processed_orderbook["MID_PRICE"] > -MID_PRICE_CUTOFF) & (processed_orderbook["MID_PRICE"] < MID_PRICE_CUTOFF)
    ]
...
def bin_and_sum(s, binwidth):
    ...
    bins = pd.interval_range(
        start=s.index[0].floor("min"), end=s.index[-1].ceil("min"), freq=pd.DateOffset(seconds=binwidth)
    )
...
def print_liquidity_stats(transacted_orders, no_bid_idx, no_ask_idx, liquidity_dropout_buffer=LIQUDITIY_DROPOUT_BUFFER):
    ...
    sys.stderr.write(
        f"TOTAL_LIQUIDITY_DROPOUTS_INSIDE_WINDOW: {buffered_total_dropouts}, ({buffer_window_length_mins}" f" mins)\n"
    )
    sys.stderr.write(
        f"TOTAL_NO_BID_EVENTS_INSIDE_WINDOW: {buffered_bid_dropouts}, ({buffer_window_length_mins}" f" mins)\n"
    )
    sys.stderr.write(
        f"TOTAL_NO_ASK_EVENTS_INSIDE_WINDOW: {buffered_ask_dropouts}, ({buffer_window_length_mins}" f" mins)\n"
    )
...
def make_plots(plot_inputs, plot_params_dict, title=None, out_file="liquidity_telemetry.png"):
    ...
    # axes[0].legend(fontsize='large')
    axes[2].set_ylabel(
        "$\\frac{\\mathrm{best\ ask\ size}}{\\mathrm{best\ ask\ size} + \\mathrm{best\ bid\ size}}$", fontsize="large"
    )
    ...
    axes[4].bar(
        plot_inputs["transacted_volume"]["center"],
        plot_inputs["transacted_volume"]["counts"],
        align="center",
        width=plot_inputs["transacted_volume"]["width"],
        fill=False,
    )
...
def main(exchange_path, ob_path, title=None, outfile="liquidity_telemetry.png", verbose=False):
    ...
    plot_inputs = {
        ...
        "liquidity_events": {"no_bid_idx": no_bid_idx, "no_ask_idx": no_ask_idx},
        "transacted_volume": {"center": center, "width": width, "counts": counts},
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CLI utility for inspecting liquidity issues and transacted volumes " "for a day of trading."
    )

    parser.add_argument(
        "stream", type=str, help="ABIDES order stream in bz2 format. " "Typical example is `ExchangeAgent.bz2`"
    )
    parser.add_argument(
        "book", type=str, help="ABIDES order book output in bz2 format. Typical example is " "ORDERBOOK_TICKER_FULL.bz2"
    )
    parser.add_argument(
        "-o",
        "--out_file",
    ...
    parser.add_argument("-t", "--plot-title", help="Title for plot", type=str, default=None)
    parser.add_argument("-v", "--verbose", help="Print some summary statistics to stderr.", action="store_true")
