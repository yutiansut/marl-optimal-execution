...
class SparseMeanRevertingOracle(MeanRevertingOracle):
    def __init__(self, mkt_open, mkt_close, symbols):
        ...
        for symbol in symbols:
            ...
            mst = self.mkt_open + pd.Timedelta(
                "{}ns".format(np.random.exponential(scale=1.0 / s["megashock_lambda_a"]))
            )
    ...
    def compute_fundamental_at_timestamp(self, ts, v_adj, symbol, pt, pv):
        ...
        v = s["random_state"].normal(
            loc=mu + (pv - mu) * (exp(-gamma * d)), scale=((theta ** 2) / (2 * gamma)) * (1 - exp(-2 * gamma * d))
        )
