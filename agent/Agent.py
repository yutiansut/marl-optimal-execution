...
class Agent:
    def __init__(self, id, name, type, random_state, log_events=False):
        ...
        if not random_state:
            raise ValueError(
                "A valid, seeded np.random.RandomState object is required " + "for every agent.Agent", self.name
            )
            sys.exit()
    ...
    def kernelStarting(self, startTime):
        ...
        log_print(
            "Agent {} ({}) requesting kernel wakeup at time {}", self.id, self.name, self.kernel.fmtTime(startTime)
        )
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        log_print("At {}, agent {} ({}) received: {}", self.kernel.fmtTime(currentTime), self.id, self.name, msg)
    ...
    def wakeup(self, currentTime):
        ...
        log_print("At {}, agent {} ({}) received wakeup.", self.kernel.fmtTime(currentTime), self.id, self.name)
