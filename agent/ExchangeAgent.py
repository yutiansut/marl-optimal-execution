...
class ExchangeAgent(FinancialAgent):
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body["msg"] in ["LIMIT_ORDER", "CANCEL_ORDER"]:
            if self.log_orders:
                self.logEvent(msg.body["msg"], js.dump(msg.body["order"], strip_privates=True))
        else:
        ...
        if msg.body["msg"] == "WHEN_MKT_OPEN":
            ...
            if symbol not in self.order_books:
                ...
                log_print(
                    "{} received QUERY_LAST_TRADE ({}) request from agent {}", self.name, symbol, msg.body["sender"]
                )
    ...
    def wide_book_warning(self):
        ...
        if self.wide_book and (self.book_freq != 0):
            log_print(
                f"WARNING: (wide_book == True) and (book_freq != 0). Orderbook will be logged in column MultiIndex "
