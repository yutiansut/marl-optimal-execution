...
class MarketReplayAgent(TradingAgent):
    def __init__(
        ...
        self.historical_orders = LOBSTEROrdersProcessor(
            self.symbol, self.date, start_time, end_time, orders_file_path, processed_orders_folder_path
        )
        ...
        if len(order) == 1:
            ...
            if not existing_order and order["SIZE"] > 0:
                self.placeLimitOrder(
                    self.symbol, order["SIZE"], order["BUY_SELL_FLAG"] == "BUY", order["PRICE"], order_id=order_id
                )
...
class LOBSTEROrdersProcessor:
    ...
    def processOrders(self):
        ...
        if os.path.isfile(processed_orders_file):
            ...
            orders_df = orders_df[LOBSTEROrdersProcessor.COLUMNS]
            orders_df["BUY_SELL_FLAG"] = (
                orders_df["BUY_SELL_FLAG"].astype(int).replace(LOBSTEROrdersProcessor.DIRECTION)
            )
            orders_df["TIMESTAMP"] = (
