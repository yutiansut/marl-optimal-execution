...
class ExecutionAgent(TradingAgent):
    ...
    def wakeup(self, currentTime):
        ...
        if self.trade:
            ...
            self.getCurrentSpread(self.symbol, depth=500)  # Zhongyao
    ...
    def placeOrders(self, currentTime):
        if currentTime == self.execution_time_horizon[-2]:
            ...
            qty = self.schedule[pd.Interval(currentTime, currentTime + datetime.timedelta(seconds=30))]  # Zhongyao
            price = ask if self.direction == "BUY" else bid
            self.placeLimitOrder(
                symbol=self.symbol, quantity=qty, is_buy_order=self.direction == "BUY", limit_price=price
            )
