...
class ZeroIntelligenceAgent(TradingAgent):
    def __init__(
        ...
        self.theta = [
            int(x)
            for x in sorted(
                np.round(self.random_state.normal(loc=0, scale=sqrt(sigma_pv), size=(q_max * 2))).tolist(), reverse=True
            )
        ]
    ...
    def kernelStopping(self):
        ...
        if self.symbol != "ETF":
            ...
            portfolio_rT, rT = self.oracle.observePortfolioPrice(
                self.symbol, self.portfolio, self.currentTime, sigma_n=0, random_state=self.random_state
            )
    ...
    def updateEstimates(self):
        ...
        obs_t = self.oracle.observePrice(
            self.symbol, self.currentTime, sigma_n=self.sigma_n, random_state=self.random_state
        )
