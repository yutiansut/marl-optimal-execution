...
class TradingAgent(FinancialAgent):
    def __init__(self, id, name, type, random_state=None, starting_cash=100000, log_events=False, log_orders=False):
        # Base class init.
        super().__init__(id, name, type, random_state, log_events=log_events)
    ...
    def kernelStarting(self, startTime):
        ...
        log_print("Agent {} requested agent of type Agent.ExchangeAgent.  Given Agent ID: {}", self.id, self.exchangeID)
    ...
    def kernelStopping(self):
        ...
        print(
            "Final holdings for {}: {}.  Marked to market: {}".format(self.name, self.fmtHoldings(self.holdings), cash)
        )
    ...
    def cancelDataSubscription(self, symbol):
        self.sendMessage(
            recipientID=self.exchangeID,
            msg=Message({"msg": "MARKET_DATA_SUBSCRIPTION_CANCELLATION", "sender": self.id, "symbol": symbol}),
        )
    ...
    def getOrderStream(self, symbol, length=1):
        self.sendMessage(
            self.exchangeID,
            Message({"msg": "QUERY_ORDER_STREAM", "sender": self.id, "symbol": symbol, "length": length}),
        )
    ...
    def placeLimitOrder(self, symbol, quantity, is_buy_order, limit_price, order_id=None, ignore_risk=True):
        ...
        if quantity > 0:
            ...
            # Log this activity.
            if self.log_orders:
                self.logEvent("ORDER_SUBMITTED", js.dump(order, strip_privates=True))
        else:
    ...
    def placeMarketOrder(self, symbol, direction, quantity, ignore_risk=True):
        ...
        if quantity > 0:
            ...
            if not ignore_risk:
                ...
                if (new_at_risk > at_risk) and (new_at_risk > self.starting_cash):
                    log_print(
                        f"TradingAgent ignored market order due to at-risk constraints: {symbol} {direction} {quantity}\n"
    ...
    def modifyOrder(self, order, newOrder):
        ...
        self.sendMessage(
            self.exchangeID, Message({"msg": "MODIFY_ORDER", "sender": self.id, "order": order, "new_order": newOrder})
        )

        # Log this activity.
        if self.log_orders:
            self.logEvent("MODIFY_ORDER", js.dump(order, strip_privates=True))
    ...
    def orderExecuted(self, order):
        ...
        # Log this activity.
        if self.log_orders:
            self.logEvent("ORDER_EXECUTED", js.dump(order, strip_privates=True))
    ...
    def orderAccepted(self, order):
        ...
        # Log this activity.
        if self.log_orders:
            self.logEvent("ORDER_ACCEPTED", js.dump(order, strip_privates=True))
    ...
    def orderCancelled(self, order):
        ...
        # Log this activity.
        if self.log_orders:
            self.logEvent("ORDER_CANCELLED", js.dump(order, strip_privates=True))
    ...
    def get_average_transaction_price(self):
        ...
        # numerator = sum(executed_order.quantity * executed_order.fill_price for executed_order in self.executed_orders)
        # denominator = sum(executed_order.quantity for executed_order in self.executed_orders)
        # try:
        #     result = round(numerator / denominator, 2)
        # except ZeroDivisionError:
        #     result = round(numerator / 0.00001, 2)
        #     print("Executed order is None")
        # return result
